...
[![Signals: 256](https://img.shields.io/badge/signals-256-brightgreen.svg)](#signals-catalog-overview)
[![License: MIT](https://img.shields.io/badge/license-MIT-purple.svg)](LICENSE)

`signalx` is a modern, production-grade Python library designed to automatically extract **256 standardized trading signals** from any OHLCV (Open, High, Low, Close, Volume) dataset. 
...
- **256 Standardized Signals across 9 Families**: Covers Trend (52), Momentum & Oscillators (39), Volatility & Breakouts (44), Volume Dynamics (32), Candlestick Formations (28), Smart Money Concepts (11), Mean Reversion (17), Statistical Metrics (20), and Multi-Indicator Composite Ensembles (13).
...
# 2. Extract all 256 standardized trading signals in coded format (default)
...
# List all 256 available signals with codes and descriptions
uv run signalx list
```
...
`signalx` provides 256 production-ready trading signals partitioned across 9 analytical families:
...
| **Mean Reversion** | 17 | `MR` | Overbought/oversold pullbacks, statistical stretch Z-scores, channel re-entries, and climax absorption | `MR001_signal` (`mr_connors_rsi2_regime_signal`), `MR003_signal` (`mr_vwap_distance_zscore_signal`), `MR004_signal` (`mr_bb_pct_b_hook_reversion_signal`) |
...
- `drop_ohlcv`: When `False` (default), returns the original DataFrame concatenated with the 256 signal columns. When `True`, returns only date/datetime columns and signal columns.
...
# Retain original OHLCV columns + 256 coded signals
full_df = signalx.generate_signals(df)

# Return only 256 coded signals + Date
